...
echo "$($PSQL "SELECT t1.name FROM games FULL JOIN teams AS t1 ON games.winner_id = t1.team_id WHERE round='Eighth-Final' AND year=2014 UNION SELECT t2.name FROM games FULL JOIN teams AS t2 ON games.opponent_id = t2.team_id WHERE round='Eighth-Final' AND year=2014")"
